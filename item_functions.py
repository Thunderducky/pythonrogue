...
def cast_lightning(*args, **kwargs):
    ...
    if target:
        results.append({'consumed': True, 'target': target, 'message': Message('A lightning bolt strikes the {0}! The damage is {1}'.format(target.name, damage))})
...
def cast_confuse(*args, **kwargs):
    ...
    for entity in entities:
        if entity.x == target_x and entity.y == target_y and entity.ai:
            ...
            results.append({'consumed': True, 'message': Message('They eyes of the {0} look vacant, as he starts to stumble around!'.format(entity.name), libtcod.light_green)})
