...
class GameMap:
    ...
    def make_map(self, max_rooms, room_min_size, room_max_size, map_width, map_height, player, entities):
        ...
        for r in range(max_rooms): #pylint: disable=unused-variable
            ...
            for other_room in rooms:
                ...
                if num_rooms == 0:
                    # if we are the first room, start the player there
                    player.x = new_x
                    player.y = new_y
                else:
                    # all rooms after the first
                    # connect it to the pervious room with a tunnel
    ...
    def place_entities(self, room, entities):
        ...
        for i in range(number_of_monsters): #pylint: disable=unused-variable 
        ...
        for i in range(number_of_items):
            ...
            if not any([entity for entity in entities if entity.x == x and entity.y ==y]):
                ...
                else: # lightening scroll
                    item_component = Item(use_function=cast_lightning, damage=40, maximum_range=5)
                    item = Entity(x,y, '#', libtcod.violet, 'Lightning Scroll', render_order=RenderOrder.ITEM, item=item_component)
