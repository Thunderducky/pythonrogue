...
def menu(con, header, options, width, screen_width, screen_height):
    ...
    # calculate total height for the header (after auto-wrap) and line per option
...
def inventory_menu(con, header, player, inventory_width, screen_width, screen_height):
    ...
    if len(player.inventory.items) == 0:
        ...
        options = [item.name for item in player.inventory.items]
        options = []
...
def main_menu(con, background_image, screen_width, screen_height):
    ...
    libtcod.console_print_ex(0, int(screen_width/2), int(screen_height/2) -4, libtcod.BKGND_NONE, libtcod.CENTER, 'TOMBS OF THE ASTRAL KINGS')
    libtcod.console_print_ex(0, int(screen_width/2), int(screen_height - 2), libtcod.BKGND_NONE, libtcod.CENTER, 'By Eric Scott')
